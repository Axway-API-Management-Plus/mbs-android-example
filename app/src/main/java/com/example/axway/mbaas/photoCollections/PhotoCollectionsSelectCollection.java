...
import com.example.axway.mbaas.LoginActivity;
...
public class PhotoCollectionsSelectCollection extends Activity {
    ...
    private class apiTask extends AsyncTask<Void, Void, JSONObject> {
        ...
        protected JSONObject doInBackground(Void... voids) {
            try {
                JSONObject userResponse = new DefaultAPI(SdkClient.getInstance()).usersShowMe(null, null);
                data.put("user_id",userResponse.getJSONObject("response").getJSONArray("users").getJSONObject(0).get("id").toString());
                successResponse = new DefaultAPI(SdkClient.getInstance()).photoCollectionsSearch(data.get("user_id").toString(),null,null,null,null);
