...
import com.example.axway.mbaas.places.PlacesQuery;
...
public class CheckinsCreate extends Activity {
	...
	private class createCheckins extends AsyncTask<Void, Void, JSONObject> {
		...
		protected JSONObject doInBackground(Void... voids) {
			...
			try {
				successResponse = new DefaultAPI(SdkClient.getInstance()).checkinsCreate(data.get("place_id").toString(),null,"Checked in to "+ placeName,null,null,
	...
	private class queryPlaces extends AsyncTask<Void, Void, JSONObject> {
		...
		protected JSONObject doInBackground(Void... voids) {
			...
			try {
				successResponse = new DefaultAPI(SdkClient.getInstance()).placesQuery(null,null,null,null,
						null,null,null,null,null,null,null);
			} catch (SdkException e) {
				exceptionThrown = e;
			}
