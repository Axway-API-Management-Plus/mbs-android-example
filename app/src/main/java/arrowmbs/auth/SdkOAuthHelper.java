...
private AuthorizationRequest getAuthorizationRequest(final @NonNull Context context, @NonNull AuthorizationServiceConfiguration serviceConfig) throws SdkException {
    ...
    if (getSelectedIDP ().getIDPName ().equals (NAME_OAUTH_IMPLICIT)) {

        authRequest = new AuthorizationRequest.Builder (serviceConfig, getSelectedIDP ().getClientId (), ResponseTypeValues.TOKEN, getSelectedIDP ().getRedirectUri ()).setScope (getSelectedIDP ().getScope ()).setState (null).setCodeVerifier (null).setGrantType (GrantTypeValues.IMPLICIT).build ();
    }
    if (getSelectedIDP ().getIDPName ().equals (NAME_OAUTH_EXPLICIT)) {
        authRequest = new AuthorizationRequest.Builder (serviceConfig, getSelectedIDP ().getClientId (), ResponseTypeValues.TOKEN, getSelectedIDP ().getRedirectUri ()).setScope (getSelectedIDP ().getScope ()).setState (null).setCodeVerifier (null).setGrantType (GrantTypeValues.IMPLICIT).build ();
        }
